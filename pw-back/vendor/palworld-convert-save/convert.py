...
def main():
    ...
    parser.add_argument("--print-json", action="store_true", help="Print JSON content to console only")
    ...
    if args.to_json or args.filename.endswith(".sav"):
        if not args.output and not args.print_json:
            output_path = args.filename + ".json"
        else:
            output_path = args.output
        convert_sav_to_json(args.filename, output_path, args.minify_json, args.print_json)
...
def convert_sav_to_json(filename, output_path, minify, print_to_console):
    ...
    json_content = json.dumps(gvas_file.dump(), indent=None if minify else "\t", cls=CustomEncoder)
